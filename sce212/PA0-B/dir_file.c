...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token){
	...
	for (int i = 0; i < num_token; i++) {
		if (strchr(token_list[i],'.')!=NULL ) {
			root_dir->file_list[root_dir->num_file]= find_create_file(root_dir, *token_list[i], &tof);
			if (tof == 1) { root_dir->num_file++; }
		}
		else {
			root_dir->dir_list[root_dir->num_dir] = find_create_dir(root_dir, *token_list[i], &tof);
			if (tof == 1) { root_dir->num_dir++; }
		}
	}
}
